    ...
    for item in data:
        ...
        try:
            ...
            if description is None:
                description = ""
            parser = MyHTMLParser()
            parser.feed(item.get("body", None))
            text = item.get("title", "") + "\n" + description + "\n" + parser.get_text()

            id = item.get("workspace_id", None) + "--" + item.get("id", None)
            source = "article"
            source_id = item.get("workspace_id", None) + "--" + item.get("id", None)
            url = item.get("url", None)
            created_at = item.get("created_at", None)
            author = item.get("author_id", None)
