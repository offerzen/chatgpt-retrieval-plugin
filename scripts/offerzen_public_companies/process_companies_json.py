...
async def process_json_dump(
    ...
    for item in data:
        ...
        try:
            ...
            id = "public_listing--" + item.get("id", None)
            source = "file"
            source_id = id
            url = "https://www.offerzen.com" + item.get("url", None)
            created_at = "1685631905"
            author = item.get("name", None)
